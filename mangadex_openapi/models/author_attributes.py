...
class AuthorAttributes(BaseModel):
    ...
    namicomi: Optional[Annotated[str, Field(strict=True)]] = None
    ...
    __properties: ClassVar[List[str]] = ["name", "imageUrl", "biography", "twitter", "pixiv", "melonBook", "fanBox", "booth", "nicoVideo", "skeb", "fantia", "tumblr", "youtube", "weibo", "naver", "namicomi", "website", "version", "createdAt", "updatedAt"]
    ...
    @field_validator('namicomi')
    def namicomi_validate_regular_expression(cls, value):
        """Validates the regular expression"""
        if value is None:
            return value

        if not re.match(r"^https?:\/\/([\w-]+\.)?namicomi\.com(\/|$)", value):
            raise ValueError(r"must validate the regular expression /^https?:\/\/([\w-]+\.)?namicomi\.com(\/|$)/")
        return value
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # set to None if namicomi (nullable) is None
        # and model_fields_set contains the field
        if self.namicomi is None and "namicomi" in self.model_fields_set:
            _dict['namicomi'] = None
